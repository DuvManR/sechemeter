...
function createBagrutRow(subject = "", units = 5, bonus = 0, isNewRow = false) {
...
function getTauBonus(subject, units, grade, selectedExamType) {
	if (grade < 60) {
		...
		return 12.5; // Math\English && 4 units
...
function isFiveUnitsMath(subjectText, examType, units, grade, isMath) {
	if (
		subjectText == "מתמטיקה" &&
		examType == "בחינה" &&
		(units < 5 || grade < 60)
...
function updateAvg() {
	// Gets the selected university option
	let university = getSelectedOption("bagrut-universities");
...
function calculateAdditionalSubjects(
	...
	sortedNoneMandatorySubjects.forEach((row) => {
		...
		if (isRowFull(units, grade, subject)) {
			...
			if (
				...
				isInvalidBagrutUnitsNum(unitsNum)
			) {
				// Bolds row's style
				row.setRowStyle("bold", "#ADD8E6");
	...
	return [unitsNum, Math.round(avg * 100) / 100];
}
...
function displayAvg(avg, unitsNum, totalUnitsNum, deltaUnits, university) {
	// Gets current university's max bagrut-average
	const maxUniversitiesBagrutAverage = {
		TAU: 117,
		HUJI: 127,
		TECH: 119,
		BGU: 120,
	...
	setBagrutResultsDivColor(totalUnitsNum, resultsDiv);
}
...
function extractBagrutTableRowData(row) {
...
function isAdditionalCalcSubject(subject, university) {
	return (
...
function shouldIncludeInAvg(subject, university, grade, bonus, currentAverage) {
...
function isInvalidBagrutUnitsNum(unitsNum) {
	return unitsNum < 20;
}

// Sets the background color of bagrut-results-div (empty => default gray color, 20- => red theme, 20+ => green theme)
function setBagrutResultsDivColor(totalUnitsNum, resultsDiv) {
	if (!totalUnitsNum) {
		resultsDiv.style.backgroundColor = "#f9f9f9"; // default gray color
	} else if (isInvalidBagrutUnitsNum(totalUnitsNum)) {
		resultsDiv.style.backgroundColor = "#ffcccc"; // red theme
	} else {
		resultsDiv.style.backgroundColor = "#D1FFBD"; // green theme
	}
}
...
var animation = lottie.loadAnimation({
	...
	path: "rotate-phone.json", // Lottie-animation .config file
});
