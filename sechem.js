...
document.getElementById("calc-button").addEventListener("click", function() {
    ...
    let selectedOption = getSelectedOption("sechem-universities");
    switch (selectedOption) {
        ...
        case 'BIU-FINAL':
            ...
            break;
        default:
            displayResults("sechem-result", "יש לבחור מוסד לימודים ❗");
    }
...
function calculateTauFinal(firstSechem, mor) {
    ...
    // Checks validity and displays results
    if (!firstSechem || !mor) {
        displayResults("sechem-result", "יש למלא את כלל השדות הקיימים ❗");
    }
    else if (isNaN(finalSechem) || mor < 150 || mor > 250 || !Number.isInteger(eval(mor))) {
        displayResults("sechem-result", "חלק מהערכים שהזנת אינם חוקיים ❗");
    }
    else {
...
function calculateHujiFirst(bagrut, psycho) {
    ...
    let normalizedBagrut = 3.9630 * decBagrut - 20.0621;
    ...
    // Checks validity and displays results
    checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, decBagrut, hujiFirstSechemTreshold, cognitiveSechem.toString())
}
...
function checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, bagrut, currentYearTreshold, results) {
    if (!psycho || !bagrut) {
        ...
        displayResults("sechem-result", "ציון הבגרות שלך נמוך מהמינימום הנדרש לחוג 😥");
        return false;
    }
    else if (psycho < min_psycho) {
        displayResults("sechem-result", "ציון הפסיכומטרי שלך נמוך מהמינימום הנדרש לחוג 😥");
        return false;
    }
...
function checkSechem(sechem, currentYearTreshold) {
    ...
    if (!currentYearTreshold && sechem != "undefined") {
        displayResults("sechem-result",  `❔ סכם: ${evalSechem} | טרם נקבע סף מינימלי לחוג זה השנה ❔`);
    }
    else {
        ...
        if (evalSechem < currentYearTreshold) {
            displayResults("sechem-result", `😥 סכם: ${evalSechem} | ${sadLog}`);
        }
        else if (evalSechem >= currentYearTreshold) {
            displayResults("sechem-result", `🎉 סכם: ${evalSechem} | ${happyLog}`);
        }
        else if (sechem == "undefined") {
            // BIU output (no SECHEM)    
            displayResults("sechem-result", `🎉 ${happyLog}`);
        }   
    }
...
document.getElementById("sechem-universities").onchange = function() {
    ...
    let selectedUniversity = getSelectedOption("sechem-universities");
...
function updateVisualChanges(topLabelText, bottomLabelText, topInputDisplay, bottomInputDisplay, sechemTableDisplay, clearBothInputs, imageSrc) {
    updateLabels(topLabelText, bottomLabelText, topInputDisplay, bottomInputDisplay);
    updateInputs(topInputDisplay, bottomInputDisplay, clearBothInputs);
    updateSechemTable(sechemTableDisplay);
    updateImage("sechem-logo-image", imageSrc);
}

// Gets the RESULTS-DIV-ID and displays the results
function displayResults(resultsElementId, results) {
...
function updateLabels(topLabelText, bottomLabelText, topLabelDisplay='block', bottomLabelDisplay='block') {
...
function updateSechemTable(sechemTableDisplay) { 
    let sechemTableElement = document.getElementById("sechem-table");
    sechemTableElement.style.display = sechemTableDisplay;

    let sechemTableLabelElement = document.getElementById("sechem-table-label");
    if (sechemTableDisplay === 'revert') {
        sechemTableLabelElement.style.display = 'block';
    }
    else {
...
function updateImage(imageElementId, imageSrc) { 
...
function extractCurrentYearTable(rawCurrentYearRow) {
    ...
    for (let i=0; i<15; i+=4){
            let beginSlice = i;
            let endSlice = i + 4;
            let currentUniTable = sliceRow(beginSlice, endSlice, rawCurrentYearRow); // Sliced Current University
...
var oldSechemTableBody = document.getElementById('sechem-table-body');
function removeOldTableAndCreateNew(selectedOption) {
    ...
    oldSechemTableBody.parentNode.replaceChild(newSechemTableBody, oldSechemTableBody);
...
const finalSechemOptions = [
    ...
    {value: 'HUJI-FINAL', text: 'העברית'},
    {value: 'TECH-FINAL', text: 'טכניון'},
    {value: 'BIU-FINAL', text: 'בר אילן'},
]
// Default frame colors (green theme)
var lastSechemPrimaryColor='#4CAF50', lastSechemSecondaryColor='#45a049', lastSechemTableBorderColor='#388e3c', lastSechemTableTextColor='#1b5e20', lastSechemTableThColor='#4caf50', lastSechemTableLinesColor='#81c784';
...
document.getElementById('sechem-switch').addEventListener('change', function() {
    ...
    if (this.checked) {
        [lastSechemPrimaryColor, lastSechemSecondaryColor, lastSechemTableBorderColor, lastSechemTableTextColor, lastSechemTableThColor, lastSechemTableLinesColor] = changeFormColors('#ff4d4d', '#b30000', '#ff0000', '#b30000', '#ffcccc', '#f28b82');
        changeSechemSelectOptions(finalSechemOptions);
    } else {
        [lastSechemPrimaryColor, lastSechemSecondaryColor, lastSechemTableBorderColor, lastSechemTableTextColor, lastSechemTableThColor, lastSechemTableLinesColor] = changeFormColors();
...
function changeFormColors(lastSechemPrimaryColor='#4CAF50', lastSechemSecondaryColor='#45a049', lastSechemTableBorderColor='#388e3c', lastSechemTableTextColor='#1b5e20', lastSechemTableThColor='#4caf50', lastSechemTableLinesColor='#81c784') {
    ...
    return [lastSechemPrimaryColor, lastSechemSecondaryColor, lastSechemTableBorderColor, lastSechemTableTextColor, lastSechemTableThColor, lastSechemTableLinesColor]
}
...
document.getElementById("to-bagrut-button").addEventListener("click", function() {
    ...
    changeFormColors('#66a3ff', '#3399ff');
});
...
document.getElementById("to-sechem-button").addEventListener("click", function() {
    ...
    changeFormColors(lastSechemPrimaryColor, lastSechemSecondaryColor, lastSechemTableBorderColor, lastSechemTableTextColor, lastSechemTableThColor, lastSechemTableLinesColor);
});
