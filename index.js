        ...
        document.getElementById("calcButton").addEventListener("click", function() {
            ...
            let bagrut = document.getElementById("bagrut").value;
            let psycho = document.getElementById("psycho").value;

            // Gets the selected option and executes the relevant function
            let selectedOption = document.getElementById("universities").value;
            switch (selectedOption) {
                case 'TAU':
                    selectedTau(bagrut, psycho);
                    break;
                case 'HUJI1':
                    selectedHuji1(bagrut, psycho);
                    break;
                case 'HUJI2':
                    selectedHuji2(bagrut, psycho); /*bagrut = cognitive sechem, psycho = morkam*/
                    break;
                case 'TECH':
                    selectedTech(bagrut, psycho);
                    break;
                case 'BGU':
                    selectedBgu(bagrut, psycho);
                    break;
                case 'BIU':
                    selectedBiu(bagrut, psycho);
                    break;
                default:
                    document.getElementById("result").innerText = "יש לבחור מוסד לימודים ❗";
            }
        ...
        // Calculates TAU SECHEM, checks if it's higher than 2024-2025 treshold
        // Checks if the bagrut & psycho grades are valid according to TAU minimum & maximums values
        function selectedTau(bagrut, psycho) {
            let min_psycho = 700
            let min_bagrut = 55
            let max_bagrut = 117
            ...
            if (checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, bagrut)) {
                sendPostRequest(url, data, headers, tauSechemTreshold);
            }
        }

        // Calculates HUJI COGNITIVE SECHEM, checks if it's higher than 2024-2025 treshold
        // Checks if the bagrut & psycho grades are valid according to HUJI minimum & maximums values
        function selectedHuji1(bagrut, psycho) {
            let min_psycho = 700
            let min_bagrut = 6
            let max_bagrut = 12.7

            // Cognitive Meshuklal Formula -->
            let B = 3.9630 * bagrut - 20.0621;
		    let P = 0.032073 * psycho + 0.3672;
		    let X = 0.3 * B + 0.7 * P;
		    let results = Math.floor((1.2235 * X - 4.4598+0.0005)*1000)/1000;

            // Checks validity and displays results
            checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, bagrut, huji1SechemTreshold, results.toString())
        }

        // Calculates HUJI FINAL SECHEM, checks if it's higher than 2024-2025 treshold
        // Checks if the COGNITIVE SECHEM & morkam grades are valid according to HUJI scheme
        function selectedHuji2(cognitive, morkam) {

            // Mor/Mirkam Standardization (Tiknun) -->
            let M = 0.0261 * morkam + 20.6791;

            // Personal Grade (Tziun Ishiuty): -->
            let sechem = Math.floor(((0.75 * M + 0.25 * cognitive) + 0.0005) * 1000) / 1000;

            // Checks validity and displays results
            if (!cognitive || !morkam) {
                document.getElementById("result").innerText = "יש למלא את כלל השדות הקיימים ❗";
            }
            else if (isNaN(sechem) || morkam < 150 || morkam > 250 || !Number.isInteger(eval(morkam)) || cognitive < 16.000 || cognitive > 30.000) {
                document.getElementById("result").innerText = "חלק מהערכים שהזנת אינם חוקיים ❗";
            }
            else {
                checkSechem(sechem, huji2SechemTreshold)
            }
        }

        // Calculates TECH SECHEM, checks if it's higher than 2024-2025 treshold
        // Checks if the bagrut & psycho grades are valid according to TECH minimum & maximums values
        function selectedTech(bagrut, psycho) {
            let min_psycho = 200
            let min_bagrut = 0
            let max_bagrut = 119

            // TECH SECHEM Formula
            let results = (0.5 * bagrut + 0.075 * psycho - 19).toFixed(3)

            // Checks validity and displays results
            checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, bagrut, techSechemTreshold, results.toString())
        }

        // Calculates BGU SECHEM, checks if it's higher than 2024-2025 treshold
        // Checks if the bagrut & psycho grades are valid according to BGU minimum & maximums values
        function selectedBgu(bagrut, psycho) {
            let min_psycho = 680
            let min_bagrut = 0
            let max_bagrut = 120
            let url = "https://cors-anywhere.herokuapp.com/https://bgu4u.bgu.ac.il/pls/rgwp/!rg.acc_SubmitSekem"
        ...
        function selectedBiu(bagrut, psycho) {
            ...
            // Checks validity and displays results
            checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, bagrut, biuSechemTreshold, results)
        }
        ...
        function sendPostRequest(url, data, headers, currentYearTreshold) {
            ...
            xmlHttp.onreadystatechange = function() { 
                ...
                if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
                    ...
                    if (response != null && !response.includes(null)) {
                        ...
                        //BGU response
                            ...
                            let sechem = response.match(/\d/g).join("")
                            checkSechem(sechem, currentYearTreshold);
                        }                    
                    }
                }
                ...
                // Raises error and solution to the user
                    ...
                    document.getElementById("result").innerText = "אם שגיאה זאת קופצת בעת חישוב הסכם של בן גוריון, יש להיכנס לקישור:\n https://cors-anywhere.herokuapp.com/corsdemo \n ללחוץ על הכפתור: \n 'Request temporary access to the demo server' \nולנסות שוב.\n (חלק ממנגנון אבטחתי של אתר האוניברסיטה - כך תתאפשר לכם גישה ל-24 שעות)";
                }
            }
        ...
        function checkGradesVal(min_psycho, min_bagrut, max_bagrut, psycho, bagrut, currentYearTreshold, results) {
            if (!psycho || !bagrut) {
                ...
                document.getElementById("result").innerText = "ציון הבגרות שלך נמוך מהמינימום הנדרש לחוג 😥";
                return false;
            }
            else if (psycho < min_psycho) {
                document.getElementById("result").innerText = "ציון הפסיכומטרי שלך נמוך מהמינימום הנדרש לחוג 😥";
                return false;
            }
        ...
        function checkSechem(sechem, currentYearTreshold) {
            ...
            if (isNaN(currentYearTreshold)) {
                document.getElementById("result").innerText = `❔ סכם: ${evalSechem} | טרם נקבע סף מינימלי לחוג זה השנה ❔`;
            }
            else {
            ...
            if (evalSechem < currentYearTreshold) {
                document.getElementById("result").innerText = `😥 סכם: ${evalSechem} | ${sadLog}`;
            }
            else if (evalSechem >= currentYearTreshold) {
                document.getElementById("result").innerText = `🎉 סכם: ${evalSechem} | ${happyLog}`;
            }
            else if (isNaN(evalSechem)) {
                // BIU output (no SECHEM)    
                document.getElementById("result").innerText = `🎉 ${happyLog}`;
            }
        }
        ...
        function updateImageAndText() {
            ...
            switch (selectedOption) {
                ...
                case 'HUJI2':
                    imageElement.src = 'data:image/png;base64,';                    
                    updateLabels(":סכם קוגנטיבי (30.000 - 16.000)", ":מורקם (250 - 150)")
                    break;
                case 'TECH':
                ...
                case 'BIU':
                    ...
                    break;
                default:
                    updateLabels(":(? - ?) ממוצע בגרות", ":פסיכומטרי (800 - 200)")
        ...
        function updateLabels(firstLabel, secondLabel) {
            document.getElementById('bagrutLabel').textContent = firstLabel;
            document.getElementById('psychoLabel').textContent = secondLabel;
        }
        ...
        function extractCurrentYearTable(currentYearRow) {
            let currentUni = "huji"; // huji/tau/tech/biu
            let currentYearTable = {"Year": currentYearRow[8], "data": {"huji": {'sechem': "", 'meshuklal': ""}, "tau": {'sechem': "", 'meshuklal': ""}, "tech": {'sechem': "", 'meshuklal': ""}, "biu": {'sechem': ""}}}
            
            for (let i=0; i<15; i+=4){
                    let beginSlice = i;
                    let endSlice = i + 4;
                    let currentUniTable = sliceRow(beginSlice, endSlice, currentYearRow)
                    currentYearTable = updateCurrentYearTable(currentUni, currentUniTable, currentYearTable)
        ...
        function filterRow(row) {
            return row.filter(cell => cell && !isNaN(cell))
        }
        ...
        function updateCurrentYearTable(currentUni, currentUniTabel, currentYearTable) {
            currentYearTable.data[currentUni].sechem = currentUniTabel[1];
            currentYearTable.data[currentUni].meshuklal = currentUniTabel[-1];
            return currentYearTable;
        }

        // Extracts old schamim's tresholds and updates variables accordingly
        var sechemTresholds;
        var tauSechemTreshold;
        var huji1SechemTreshold;
        var huji2SechemTreshold;
        var techSechemTreshold;
        var bguSechemTreshold = 735;
        var biuSechemTreshold = 0;
        var currentYear;
        (async () => {
            sechemTresholds = await carveSechemTresholds();
            tauSechemTreshold = sechemTresholds[0].data.tau.sechem;
            huji1SechemTreshold = sechemTresholds[0].data.huji.sechem;
            huji2SechemTreshold = sechemTresholds[0].data.huji.meshuklal;
            techSechemTreshold = sechemTresholds[0].data.tech.sechem;
